	...
	public void test1AddCustomer() {
		...
		try {
			...
			if (sessionID == -1) {
				...
				System.out.println("============"+project.sessions);
				custIDs[0] = project.addCustomer(customers[0], custCities[0],
						streets[0], sessionID);
				
				System.out.println("My print==================="+customers[0]
						+" "+custCities[0]+" "+streets[0]+" "+custIDs[0]);
				project.logout(sessionID);
				result += "ok!";
				System.out.println(result);
			}
		...
		try {
			...
			if (sessionID == -1) {
				...
				for (int i = 0; i < edges.length; i++)
					project.addRoute(edges[i][0], edges[i][1],
							100 * weights[i], sessionID);
				project.logout(sessionID);
				result += "ok!";
				System.out.println(result);
			}
	...
	public void test1AddProduct() {
		...
		try {
			...
			if (sessionID == -1) {
				...
				prodID = project.addProduct(products[3], catIDs[1], 1.0,
						sessionID);
			project.logout(sessionID);
			if (prodID == -1) {
				result += "ok!";
			...
			if (sessionID == -1) {
				...
				for (int i = 0; i < products.length; i++) {
					if (i < 3)
						...
						catID = catIDs[1];
					prodID = project.addProduct(products[i], catID, 1.0,
							sessionID);
					if (prodID == -1)
						errCtr++;
					else
